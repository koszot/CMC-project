...
fasta <- read_tsv("../../AS_project/FILES_ltigrinus_v2/ENRICHMENT_ltigrinus/Lenti6_1_GeneCatalog_proteins_20130903.aa.fasta.headers", col_names = c("fasta_transcript_id", "gene_id"))
# beolvassuk az eredeti annotációt ami alapján az expressziós analyzis készült
original <- read_tsv("../../AS_project/FILES_ltigrinus_v2/GENOME_ltigrinus/Lenti6_1_GeneCatalog_genes_20130903.gff", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand",  "att2", "attributes"))
original <- original %>%
  filter(type == "exon") %>%
...
devreg$CMC_gene_id <- str_replace(devreg$CMC_gene_id, "^", "Lenti6_1_")
...
init$CMC_gene_id <- str_replace(init$CMC_gene_id, "^", "Lenti6_1_")
...
as$CMC_gene_id <- str_replace(as$CMC_gene_id, "^", "Lenti6_1_")
...
devreg_as$CMC_gene_id <- str_replace(devreg_as$CMC_gene_id, "^", "Lenti6_1_")
...
devreg_as_devreg$CMC_gene_id <- str_replace(devreg_as_devreg$CMC_gene_id, "^", "Lenti6_1_")
...
NOdevreg_as$CMC_gene_id <- str_replace(NOdevreg_as$CMC_gene_id, "^", "Lenti6_1_")
...
NOdevreg_as_devreg$CMC_gene_id <- str_replace(NOdevreg_as_devreg$CMC_gene_id, "^", "Lenti6_1_")
...
init_as$CMC_gene_id <- str_replace(init_as$CMC_gene_id, "^", "Lenti6_1_")
...
init_as_init$CMC_gene_id <- str_replace(init_as_init$CMC_gene_id, "^", "Lenti6_1_")
...
NOinit_as$CMC_gene_id <- str_replace(NOinit_as$CMC_gene_id, "^", "Lenti6_1_")
...
NOinit_as_init$CMC_gene_id <- str_replace(NOinit_as_init$CMC_gene_id, "^", "Lenti6_1_")
