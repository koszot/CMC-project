...
TxDb <- makeTxDbFromGFF(file="aostoyae_AS_genes.gtf", format="gtf")
...
TxDb <- makeTxDbFromGFF(file="aostoyae_FB_DEVREG_genes.gtf", format="gtf")
features <- binGenome(TxDb)
AS_count <- read_lines("ASpli_binFeatures.log")
AS_count <- as_tibble(AS_count[-c(1,2,3,4,5,6,7,8)]) %>%
  separate(value, c("l1", "l2", "l3", "l4", "l5"), sep = "\t") %>%
...
TxDb <- makeTxDbFromGFF(file="aostoyae_FB_INIT_genes.gtf", format="gtf")
features <- binGenome(TxDb)
AS_count <- read_lines("ASpli_binFeatures.log")
AS_count <- as_tibble(AS_count[-c(1,2,3,4,5,6,7,8)]) %>%
  separate(value, c("l1", "l2", "l3", "l4", "l5"), sep = "\t") %>%
...
for (x in 1:length(unique(FB_devreg_genes$gene_id))) {
  temp <- FB_devreg_genes[FB_devreg_genes$gene_id %in% unique(FB_devreg_genes$gene_id)[x],]
...
colnames(stats) <- c("Description", "Armillaria ostoyae")
