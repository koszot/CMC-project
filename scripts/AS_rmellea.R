...
stats <- read_tsv("rmellea_stats.tsv")

##### AS GENES #####

TxDb <- makeTxDbFromGFF(file="rmellea_AS_genes.gtf", format="gtf")
...
TxDb <- makeTxDbFromGFF(file="rmellea_FB_DEVREG_genes.gtf", format="gtf")
features <- binGenome(TxDb)
AS_count <- read_lines("ASpli_binFeatures.log")
AS_count <- as_tibble(AS_count[-c(1,2,3,4,5,6,7,8)]) %>%
  separate(value, c("l1", "l2", "l3", "l4", "l5"), sep = "\t") %>%
...
TxDb <- makeTxDbFromGFF(file="rmellea_FB_INIT_genes.gtf", format="gtf")
features <- binGenome(TxDb)
AS_count <- read_lines("ASpli_binFeatures.log")
AS_count <- as_tibble(AS_count[-c(1,2,3,4,5,6,7,8)]) %>%
  separate(value, c("l1", "l2", "l3", "l4", "l5"), sep = "\t") %>%
...
colnames(stats) <- c("names", "rmellea", "%_rmellea")
